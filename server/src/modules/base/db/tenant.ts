...
export class TenantSubscriber implements EntitySubscriberInterface<any> {
  ...
  @Config('cool.tenant.ignoreUsername')
  ignoreUsernameConfig: string[];
   ...
   */
  getIgnoreUsername() {
    if (Array.isArray(this.ignoreUsernameConfig)) {
      this.ignoreUsername = this.ignoreUsernameConfig;
    }
  ...
  getTenantId(): number | undefined {
    ...
    url = ctx?.url;
    // 忽略用户
    if (this.getIgnoreUsername().includes(ctx?.admin?.username)) {
      return undefined;
    }
    // 忽略系统接口
    if (
